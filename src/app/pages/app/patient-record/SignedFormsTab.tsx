...
export function SignedFormsTab() {
  ...
  if (patient.signedForms.length === 0) {
    return <div className="px-4 py-4 text-center text-ink-muted italic">No forms on file.</div>;
  }
      ...
      <div className="rounded-card bg-surface overflow-hidden">
        <table className="w-full text-sm text-left">
          <thead className="bg-surface-page border-b border-divider text-ink-soft">
            <tr>
              <th className="px-3 py-2.5 font-semibold">Form Name</th>
              <th className="px-3 py-2.5 font-semibold">Type</th>
              <th className="px-3 py-2.5 font-semibold">Version</th>
              <th className="px-3 py-2.5 font-semibold">Status</th>
              <th className="px-3 py-2.5 font-semibold">Signed Date</th>
              <th className="px-3 py-2.5 font-semibold">Signed By</th>
            ...
            {patient.signedForms.map((f) => {
              ...
              return (
                <tr key={f.id} className="hover:bg-surface-hover">
                  ...
                  <td className="px-3 py-2.5 text-ink-soft">{f.type}</td>
                  <td className="px-3 py-2.5 text-ink-muted font-mono text-xs">{f.version}</td>
                  <td className="px-3 py-2.5"><StatusPill status={f.status} type={formStatusPillType(f.status)} /></td>
                  <td className="px-3 py-2.5 text-ink-soft">{f.signedDate ?? "—"}</td>
                  <td className="px-3 py-2.5 text-ink-soft">{f.signedBy ?? "—"}</td>
