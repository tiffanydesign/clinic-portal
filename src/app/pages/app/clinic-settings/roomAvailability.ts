...
export function roomUtilisationPct(room: Room, appts: Appt[]): number {
  const open = clinicOpenMinutesPerDay();
  if (open <= 0) return 0;
...
export function clinicUtilisationPct(rooms: Room[], appts: Appt[]): number {
  const open = clinicOpenMinutesPerDay() * rooms.length;
  if (open <= 0) return 0;
