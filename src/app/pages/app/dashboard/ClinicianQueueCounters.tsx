...
function CounterButton({ label, value, sublabel, hasOverdue, onClick }: {
  ...
  return (
    <button
      onClick={onClick}
      className="flex-1 min-w-0 text-left px-3 py-3 hover:bg-gray-50 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-slate-400 focus-visible:ring-inset"
    >
...
function CounterTile({ label, value, hasOverdue }: { label: string; value: number; hasOverdue?: boolean }) {
  return (
    <div className="flex-1 min-w-0 text-left px-3 py-3">
      <span className="flex items-center gap-1.5">
        <span className="text-[11px] font-bold text-gray-500 uppercase whitespace-nowrap leading-tight">{label}</span>
        {hasOverdue && <span className="w-1.5 h-1.5 rounded-full bg-red-500 shrink-0" aria-hidden />}
      </span>
