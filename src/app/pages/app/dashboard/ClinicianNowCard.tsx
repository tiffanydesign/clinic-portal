...
import { AlertTriangle, Video, MapPin, CheckCircle2 } from "lucide-react";
import { Appt, JOURNEY_STEPS_RECEPTION } from "./dashboardData";
import { JourneyDots } from "./AppointmentDrawer";
...
export function ClinicianNowCard({
  ...
  if (activeAppt) {
    const a = activeAppt;
    return (
      <div className="border border-orange-200 bg-orange-50/60 rounded-xl shadow-sm p-5 shrink-0">
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {a.patient.alert && (
                <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider bg-red-50 border border-red-200 text-red-700 shrink-0">
                  <AlertTriangle className="w-3 h-3" /> {a.patient.alert}
                </span>
              )}
            </div>
  ...
  if (upNextAppt) {
    ...
    return (
      <div className="border border-blue-200 bg-blue-50/50 rounded-xl shadow-sm p-5 shrink-0">
        <div className="flex items-center gap-2 mb-3">
        ...
        {a.isVideo ? (
          ...
          <div className="mt-4 px-1">
            <JourneyDots steps={JOURNEY_STEPS_RECEPTION} current={a.currentStep} />
          </div>
        )}
  ...
  return (
    <div className="border border-gray-200 bg-white rounded-xl shadow-sm p-6 shrink-0 flex items-center gap-3">
