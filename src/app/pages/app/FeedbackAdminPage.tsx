...
function FeedbackCard({ item, compact, isSelected, onClick }: {
  ...
  if (compact) {
    return (
      ...
      >
        ...
        {item.flagged && <Flag className="w-3 h-3 text-danger-ink fill-current shrink-0" />}
  ...
  return (
    ...
    >
      ...
      {item.flagged && <Flag className="w-3.5 h-3.5 text-danger-ink fill-current shrink-0" />}
...
function FeedbackDrawer({ item, onClose, newNote, setNewNote, onAddNote, onUpdateStatus, onToggleFlag, onMarkAddressed }: {
  ...
  return (
    <div className="fixed inset-y-0 right-0 w-[560px] bg-surface border-l border-divider shadow-2xl flex flex-col animate-in slide-in-from-right duration-200 z-50">
      ...
      <div className="px-6 py-4 bg-surface border-t border-divider flex items-center justify-between shrink-0 gap-2">
        <button
          ...
          className={`flex items-center px-4 py-2 text-sm font-bold rounded-card transition-colors border shrink-0
            ${item.flagged ? "bg-danger/10 text-danger-ink border-danger/30" : "bg-surface text-ink-soft border-divider hover:bg-surface-hover"}`}
        >
...
export function FeedbackAdminPage() {
  ...
  useEffect(() => { setPage(1); }, [activeTab, sourceFilter, typeFilter, statusFilter, searchQuery]);
  ...
  return (
    <div className="flex flex-col h-full bg-surface-page overflow-hidden">
      ...
      <div className="flex-1 relative overflow-hidden border-t border-divider">
        <div
          className={`h-full flex flex-col transition-[margin-right] duration-300 ease-out ${selectedItem ? "mr-[560px]" : "mr-0"}`}
        ...
        >
          ...
          <Pagination page={page} pageCount={pageCount} totalCount={filtered.length} onPageChange={setPage} itemLabel="records" />
        </div>
