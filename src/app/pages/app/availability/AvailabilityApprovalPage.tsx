...
function OverviewTile({ icon: Icon, value, label, iconClass }: {
  ...
  return (
    <div className="flex items-center gap-3 px-5 py-3.5 min-w-0">
      <div className={`w-10 h-10 rounded-card flex items-center justify-center shrink-0 ${iconClass}`}>
        <Icon className="w-5 h-5" />
      </div>
      <div className="min-w-0">
        <div className="text-2xl font-bold text-ink tabular-nums leading-none">{value}</div>
        <div className="text-label font-bold text-ink-muted uppercase tracking-wide mt-1 truncate">{label}</div>
      </div>
    </div>
  );
}
    ...
    >
      ...
      <span className="flex-1 min-w-0">
        ...
        <span className="flex items-center gap-1.5 mt-1 text-xs text-ink-muted">
          ...
          <span className={`ml-1 px-1.5 py-0.5 text-label font-semibold rounded-control border shrink-0 ${reasonBadgeClass(leave.reason)}`}>{reasonText(leave)}</span>
        </span>
      </span>
      {conflicts > 0 && (
        <span className="hidden sm:flex items-center gap-1 px-2 py-0.5 rounded-full bg-danger/10 text-danger-ink text-label font-bold shrink-0">
          <AlertTriangle className="w-3 h-3" /> {conflicts}
        </span>
      )}
      <span className="text-xs text-ink-muted shrink-0 tabular-nums hidden md:block">{leave.submittedAt}</span>
      <ChevronRight className="w-4 h-4 text-ink-muted group-hover:text-ink-muted shrink-0 transition-colors" />
    </button>
  );
}
...
function ConflictList({ conflicts, onResolve }: { conflicts: BookedAppt[]; onResolve: (index: number) => void }) {
  return (
    <div className="space-y-2 mt-3">
      {conflicts.map((c, i) => (
        <div key={i} className={`rounded-card border px-3 py-2.5 ${c.resolved ? "bg-success/10 border-success/30" : "bg-surface border-danger/30"}`}>
          <div className="flex items-start justify-between gap-3">
            <span className={`text-sm font-medium ${c.resolved ? "text-success-ink line-through" : "text-ink-soft"}`}>{bookingLabel(c)}</span>
            {c.resolved && <span className="text-overline text-success-ink shrink-0 mt-0.5">Resolved</span>}
          </div>
          {!c.resolved && (
...
function DetailRow({ icon: Icon, label, children }: {
  ...
  return (
    <div className="flex items-start gap-3 py-3 border-b border-divider last:border-0">
      ...
      <div className="min-w-0">
        <div className="text-label font-bold text-ink-muted uppercase tracking-wider">{label}</div>
...
function LeaveDrawer({ leave, onClose, onApprove, onReject, onResolve }: {
  ...
  return (
    <div className="fixed inset-0 z-50">
      <div
        className="absolute inset-0 bg-surface-sunken/30 backdrop-blur-sm animate-in fade-in duration-200"
        onClick={onClose}
      />
      <aside
        role="dialog"
        ...
        className="absolute right-0 inset-y-0 w-full sm:max-w-md bg-surface shadow-2xl border-l border-divider flex flex-col animate-in slide-in-from-right duration-300"
      >
        {/* Header */}
        <div className="px-6 py-5 border-b border-divider shrink-0">
          <div className="flex items-start justify-between gap-3">
          ...
          <div className="flex items-center gap-3 mt-3">
            <span className="w-11 h-11 rounded-full bg-surface-sunken text-ink-soft flex items-center justify-center text-sm font-bold shrink-0">{EMPLOYEE_INITIALS}</span>
            <div className="min-w-0">
              <div className="text-base font-bold text-ink truncate">{EMPLOYEE_NAME}</div>
              <div className="text-xs text-ink-muted">{EMPLOYEE_ROLE}</div>
            </div>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-2">
          ...
          <DetailRow icon={FileText} label="Reason">
            <span className={`inline-flex items-center px-2 py-0.5 text-xs font-semibold rounded-control border ${reasonBadgeClass(leave.reason)}`}>{reasonText(leave)}</span>
          </DetailRow>
          <DetailRow icon={CalendarClock} label="Submitted">{leave.submittedAt}</DetailRow>

          {/* Conflicts */}
          {leave.conflicts.length > 0 && (
            <div className="mt-4 rounded-card border border-danger/30 bg-danger/10 p-4">
              <div className="flex items-center gap-1.5 text-xs font-bold text-danger-ink uppercase tracking-wider">
                <AlertTriangle className="w-3.5 h-3.5" />
                {leave.conflicts.length} affected booking{leave.conflicts.length === 1 ? "" : "s"}
              </div>
              <p className="text-xs text-danger-ink/80 mt-1">Reschedule or cancel each booking before this request can be approved.</p>
        ...
        <div className="px-6 py-4 border-t border-divider bg-surface-page shrink-0">
          {approveDisabled && (
            <p className="text-label text-warning-ink font-medium mb-2 flex items-center gap-1">
          ...
          <div className="flex gap-3">
            <button
              onClick={onReject}
              className="flex-1 inline-flex items-center justify-center gap-1.5 px-4 py-2.5 border border-danger/30 bg-surface text-danger-ink text-sm font-bold rounded-card hover:bg-danger/10 transition-colors"
            >
            ...
            <button
              ...
              className={`flex-1 inline-flex items-center justify-center gap-1.5 px-4 py-2.5 text-sm font-bold rounded-card transition-colors ${
                approveDisabled ? "bg-surface-sunken text-ink-muted cursor-not-allowed" : "bg-success-ink text-white hover:bg-success-ink"
              }`}
            >
    ...
    <div className="p-4 h-full overflow-y-auto">
      ...
      <div className="grid grid-cols-3 divide-x divide-divider bg-surface rounded-card mb-6">
        ...
        <OverviewTile
          ...
          iconClass={needsAttention > 0 ? "bg-danger/10 text-danger-ink" : "bg-success/10 text-success-ink"}
        />
        <OverviewTile icon={ClipboardList} value={decidedCount} label="Decided" iconClass="bg-surface-hover text-ink-soft" />
      </div>

      {/* L3 — Pending queue */}
      <div className="flex items-center justify-between mb-2">
        <h2 className="text-sm font-bold text-ink-muted uppercase tracking-wider">Pending requests</h2>
        {awaiting > 0 && <span className="text-xs font-medium text-ink-muted">{awaiting} total</span>}
      </div>
      ...
      {awaiting === 0 ? (
        <div className="rounded-card bg-surface py-6 flex flex-col items-center justify-center text-center">
          ...
          <div className="text-sm font-bold text-ink-soft">You're all caught up</div>
            ...
            {store.decisions.slice(0, 5).map((d) => (
              <div key={d.id} className="px-4 py-3 flex items-center gap-3">
                ...
                <div className="flex items-center gap-2 min-w-0 flex-1">
                  <span className={`px-1.5 py-0.5 text-overline rounded-control border shrink-0 ${kindBadgeClass(d.kind)}`}>{d.kind}</span>
                  <span className="text-sm text-ink-soft truncate">{d.summary}</span>
                </div>
                <span className={`text-overline shrink-0 ${d.result === "Approved" ? "text-success-ink" : "text-danger-ink"}`}>{d.result}</span>
              </div>
            ))}
          </div>
        </div>
